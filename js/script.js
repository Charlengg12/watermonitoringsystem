...
function fetchReadings() {
    ...
    // Attempt to fetch data from PHP backend
    fetch(SERVER_URL)
        .then(response => {
            // Check for the 503 error (No data available yet)
            if (response.status === 503) {
                updateGlobalStatus("Service Unavailable (503): Waiting for ESP32 to send data.");
