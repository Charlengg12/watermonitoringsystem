...
// Fetch user info
$query = $conn->prepare("SELECT fullname, email, number, profile_pic FROM users WHERE user_id = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['field'], $_POST['value'])) {
    ...
    if (in_array($field, ['fullname', 'email', 'number'])) {
  ...
  <style>
    ...
    .info-text {
       background: #2a2f3a;
        ...
        display: block;
        font-size: 18px;
        width: 70%;          /* consistent box size */
        max-width: 500px;    /* optional: prevent too wide */
    ...
    .btn-change {
      ...
      display: block;
      margin: 12px auto 0 auto; /* center below text */
    }
    .btn-change:hover {
      background: #009ad9;
      color: #fff;
...
<body>
  ...
  <div class="container d-flex justify-content-center">
    <div class="container-box">
      ...
      <div class="info-row">
        ...
        <button class="btn-change" data-bs-toggle="modal" data-bs-target="#editNumberModal">Change Contact</button>
      </div>
    </div>
  </div>
  ...
  <div class="modal fade" id="editNumberModal" tabindex="-1">
    <div class="modal-dialog">
      <form method="POST" class="modal-content">
        <div class="modal-header border-0">
          <h5 class="modal-title"><i class="fas fa-phone me-2"></i> Change Contact</h5>
          <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
        </div>
        <div class="modal-body">
